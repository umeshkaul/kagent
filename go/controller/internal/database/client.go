...
type Client interface {
	...
	UpdateSession(sessionName string, userID string, session *Session) (*Session, error)
	UpdateToolServer(server *ToolServer) error
	UpdateRun(run *Run) error
	GetMessagesForRun(runID uint) ([]Message, error)
}
...
func (c *clientImpl) UpdateSession(sessionName string, userID string, session *Session) (*Session, error) {
	err := c.serviceWrapper.Session.Update(session)
	if err != nil {
		return nil, err
	}
	return session, nil
...
// GetMessagesForRun retrieves messages for a specific run (helper method)
func (c *clientImpl) GetMessagesForRun(runID uint) ([]Message, error) {
